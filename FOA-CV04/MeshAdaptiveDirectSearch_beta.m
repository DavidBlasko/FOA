...
% % NOT WORKIN WELL PSEUDO FROM BOOK
% function D = rand_positive_spanning_set(alpha, n) 
%     delta = round(1 / sqrt(alpha));  % Step size scaling
%     L = diag(delta * (2 * rand(n,1) - 1)); % Random diagonal matrix with values scaled by delta
% 
%     % Fill lower triangular part with random values
%     for i = 1:n-1
%         for j = 1:i-1
%             L(i,j) = rand * (2 * delta - 2) + (1 - delta); % Random value in range [-delta, delta]
%         end
%     end
% 
%     % Randomly permute rows and columns
