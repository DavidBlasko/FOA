...
%% Conjugate Gradient w/ Optimal Stepsize & Polak-Ribiere
function [traj, iter_count] = conjugate_gradient_optimal_stepsize(x0, max_iter, tol)
    x = x0;
    traj = x;
    grad = gradient(x);
    d = -grad;
...
xlabel('x_1'); ylabel('x_2'); title('Conjugate Gradient w/ Optimal Stepsize');

[traj_conj_grad_optimal, iter_count] = conjugate_gradient_optimal_stepsize(x0, max_iter, tol);
plot(traj_conj_grad_optimal(1, :), traj_conj_grad_optimal(2, :), 'rx-', 'DisplayName', 'Trajectory');

legend;

%% Printing
fprintf('Optimization completed in %d iterations.\n', iter_count);
fprintf('Optimum: x = [%f, %f], f(x) = %f\n', traj_conj_grad_optimal(1, end), traj_conj_grad_optimal(2, end), rosenbrock(traj_conj_grad_optimal(:, end)));
